...
function updateDisplay(a){var b=$("#top"),c=$("#bottom");"Init"===a.state?(b.text("Initialising"),c.text(session)):"Active"===a.state?(b.text(a.l1),c.text(a.l2)):(b.text(""),c.text(""));$(".scenechange").removeClass("active");b=sceneToProgram(a.scene);$("#scene-"+b.toString()).addClass("active");current_state=a.state;scene_index=b;a=adjacentPrograms();null===a.prev?$("#previous").prop("disabled",!0):$("#previous").prop("disabled",!1);null===a.next?$("#next").prop("disabled",!0):$("#next").prop("disabled",
!1);"Blank"===current_state?$("#blank").html('<i class="fas fa-play h1" title="Un-Blank"></i>'):$("#blank").html('<i class="fas fa-pause h1" title="Blank"></i>')}function showblank(){locked||("Blank"===current_state?send(order.get(scene_index),"Active"):send(order.get(scene_index),"Blank"))}function scenechange(a){locked||(scene_index=a,direct(order.get(a)))}function showprevious(){if(!locked){var a=adjacentPrograms().prev;null!==a&&(scene_index=a,direct(order.get(a)))}}
...
function send(a,b){$.ajax({url:"/api/update/",method:"POST",dataType:"json",data:{session:session,key:api_key,scene:a,state:b},success:updateDisplay})}function check(){$.ajax({url:"/api/session_state/",dataType:"json",type:"get",data:{session:session},success:updateDisplay})};
